...
function approve() {
    ...
    var spendToken=web3.utils.toWei(trxspenddoc.value)/1e9
   approveBUSD(spendToken.toString());
}
...
function refreshData(){
    ...
    contractBalance(function(result){
        ...
        const tokenBal = stripDecimals(rawStr, 3) + ' PulseFeg';
    ...
    userBalance(function(result){
        rawStr = numberWithCommas(Number(result*1e9).toFixed(3));
        userBalanceElem.textContent = stripDecimals(rawStr, 3) + ' PulseFeg';
    });
...
function updateBuyPrice(){
    ...
    calculateEggBuySimple(spendToken.toString(),function(eggs){
...
function formatTrxValue(trxstr){
    return parseFloat((parseFloat(trxstr)*1e9).toFixed(0));
}
